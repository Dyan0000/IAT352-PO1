		...
		<section class="main">
			...
			<div class="filter_section col-1-3">
				...
				<div class="ratings">
					...
					<?php
					 ...
					 while ($row = mysqli_fetch_assoc($result)) {
					 	if (!empty($row['meat'])) 
					 		echo "<label><input type='checkbox' value='". $row['meat']. "'>". $row['meat']. "</label>";
					 }
					?>
				</div>
				...
				<div class="dietary">
					<h2 class="filter_title">Dietary</h2>
					<?php
					 $query = "SELECT DISTINCT dietary FROM dishes ORDER BY dietary ASC";
					 $result = mysqli_query($db,$query);
					 while ($row = mysqli_fetch_assoc($result)) {
					 	if (!empty($row['dietary'])) 
					 		echo "<label><input type='checkbox' value='". $row['dietary']. "'>". $row['dietary']. "</label>";
					 }
					?>
				</div>
